...
| 스킬 | 언제 쓰나 | 남기는 것 |
| --- | --- | --- |
| `superloopy-loop` | `loopy <task>`, `loopy team <task>`, `loopywork`, `$lpy`로 시작했거나, 근거가 있어야 끝낼 수 있는 작업. | 가벼운 계획, 다음 행동, 명령으로 검증한 근거, 품질 게이트, 최종 evidence report. |
...
기본 안전장치는 loop 스킬입니다. research와 clone은 명시적으로 켜는 전문 모드이고, 둘 다 마지막에는 말로 끝났다고 하지 않고 Superloopy evidence를 남깁니다.
