...
Loopy-native boundary: public names, docs, tests, and runtime modules describe Loopy behavior directly. Loopy does not vendor external harness code, generated runtime state, or source-branded role names.
...
| `.codex/agents/zoro.toml` | Project-scoped Codex reviewer agent for bounded Loopy subagent workflows. | Loopy-native report contract; no product edits and no external workflow dependency. |
| `.codex/agents/franky.toml` | Project-scoped Codex executor agent for one criterion or independent slice. | Loopy-native handoff and evidence receipt contract only. |
| `.codex/agents/jinbe.toml` | Project-scoped Codex gate reviewer for final evidence integration. | Uses Loopy evidence, review, QA, audit, and criteria coverage as the authority. |
| `.codex/agents/usopp.toml` | Project-scoped Codex QA agent for artifact-backed scenario evidence. | Loopy-native QA report contract; no product edits. |
...
| `manual-ko.pdf` | Korean field manual artifact for Loopy usage. | Documentation artifact only; no executable plugin logic. |
| `manual.pdf` | Field manual artifact for Loopy usage. | Documentation artifact only; no executable plugin logic. |
| `README.md` | Public product overview, marketplace install flow, bootstrap behavior, command flow, state model, hooks, gates, and doctor checks. | Uses Loopy-native product terms; one explicit GitHub install-pattern reference is allowed and audited. |
...
| `.codex/agents/robin.toml` | Read-only skeptical evidence-auditor agent, installed so the host can spawn the robin auditor by name. | Loopy-native auditor contract; host supplies the model. |
| `.codex/agents/nami.toml` | Read-only codebase-navigator agent for subagent-driven work; locates files and code and returns absolute paths, writing no evidence receipt. | Loopy-native read-only search contract; no product edits and no host coupling. |
...
| `src/check.js` | Non-mutating evidence preflight with summary counts, repair plan, and exact commands. | Loopy-only strictness layer. |
| `src/cli.js` | Single command dispatcher for install, loop, bin, agents, doctor, and hook entrypoints; supports symlinked bin execution. | Public doctor scan uses generic comparison paths; install writes only local wrapper and agent files. |
...
| `src/doctor.js` | Local health check orchestrator for package, hooks, docs, vocabulary, comparison scan, and reviewability. | Enforces native vocabulary and file audits. |
...
| `src/fleet.js` | Parent-side subagent coordination: a handoff registry and fleet reconciliation that normalizes the three worker verdict vocabularies into one enum. | Loopy-native; parent-side only, never spawns or completes. |
...
| `src/hooks.js` | Codex hook runtime for bootstrap, guards, receipts, guide-backed context, continuation, and steering. | Loopy hook messages, setup paths, and evidence roots. |
...
| `src/report.js` | Writes portable markdown evidence reports with Evidence Summary section and next action. | Loopy-only reporting layer. |
...
| `src/trace.js` | Builds compact evidence trail with summary counts, missing proof, suggested paths, and timeline. | Loopy-only inspection surface. |
...
| `test/doctor.test.js` | Doctor coverage for package, audit, vocabulary, comparison, design audit, and reviewability checks. | Uses synthetic fixtures only. |
| `test/file-audit.test.js` | Direct unit coverage for row-level file audit verifier. | Tests Loopy audit parser. |
| `test/fleet.test.js` | Handoff registry, fleet reconciliation, and verdict-normalization tests. | Tests Loopy parent-side coordination. |
...
| `test/plugin.test.js` | Plugin manifest and hook route tests. | Verifies Loopy packaging. |
...
- `loopy doctor --json` checks row-level inventory coverage for every Git-visible repository file, ignores Codex's generated `.codex-marketplace-install.json` cache metadata, fails stale audit rows, verifies role and compatibility-boundary columns, checks gate notes, checks the design audit, scans native vocabulary with the audited README install-reference exception, and verifies reviewable file sizes.
