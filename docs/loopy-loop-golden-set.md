...
This is Loopy's own long-running loop-engineering golden set. It scores Loopy on explicit criteria, artifact-backed proof, strict continuation, local state, append-only history, native vocabulary, and reviewable files.
...
| Score | Meaning |
| ---: | --- |
| 20 | Source-branded markers are known but not reduced. |
| 40 | Runtime names are partially native, docs/tests still expose lineage. |
| 60 | Runtime is mostly native, but public docs or tests still carry source-branded smell. |
| 80 | Public surface is Loopy-native and compatibility behavior remains covered. |
| 100 | Fresh audit proves no source-branded markers remain, doctor passes, and full tests pass. |
...
| --- | ---: | --- |
| Native vocabulary | 25 | Git-visible files avoid source-branded markers. |
...
| `.codex/agents/zoro.toml` | `test/docs.test.js`, audit coverage. | Must define the Loopy code-review role, active evidence root, and `LOOPY_EVIDENCE` receipt. |
| `.codex/agents/franky.toml` | `test/docs.test.js`, audit coverage. | Must define the bounded Loopy executor role, active evidence root, and `LOOPY_EVIDENCE` receipt. |
| `.codex/agents/jinbe.toml` | `test/docs.test.js`, audit coverage. | Must define the Loopy gate-review role, active evidence root, and `LOOPY_EVIDENCE` receipt. |
| `.codex/agents/usopp.toml` | `test/docs.test.js`, audit coverage. | Must define the Loopy QA role, active evidence root, and `LOOPY_EVIDENCE` receipt. |
| `.codex-plugin/plugin.json` | `test/plugin.test.js`, doctor plugin manifest check. | Must expose `./skills/` and the Loopy hook files only. |
...
| `README.md` | `test/docs.test.js` public-doc assertions. | Must describe actual Loopy install, bootstrap, commands, evidence rules, hooks, gates, and doctor checks; only the audited GitHub install-pattern reference may name an external source. |
...
| `hooks/stop.json` | Optional hook file, direct hook tests. | Must route Stop continuation through the Loopy CLI. |
...
| `.codex/agents/robin.toml` | `test/cli.test.js`, `test/doctor.test.js`, audit coverage. | Must define the robin auditor role and the LOOPY_AUDIT verdict receipt; installed via LOOPY_AGENT_NAMES. |
| `.codex/agents/nami.toml` | `test/cli.test.js`, `test/doctor.test.js`, audit coverage. | Must define the read-only navigator role, return absolute paths, and write no evidence receipt; installed via LOOPY_AGENT_NAMES and matched by a SubagentStop matcher. |
...
| `src/check.js` | Loop-gate and CLI evidence tests. | Must be non-mutating and print repair commands for every unresolved or invalid proof. |
| `src/cli.js` | CLI, plugin, and doctor tests. | Must dispatch install, loop, bin, agents, doctor, hook commands, generic comparison-check flags, and symlinked bin execution. |
...
| `src/doctor.js` | `test/doctor.test.js`, `node src/cli.js doctor --json`. | Must verify package, hooks, skill, audit, vocabulary, comparison status, and reviewability while ignoring generated Codex marketplace install metadata. |
...
| `src/fleet.js` | `test/fleet.test.js`. | Must record handoffs under a lock, reconcile dispatched-vs-outstanding, and normalize APPROVE/PASS/REJECT-style verdicts into one enum. |
...
| `src/hooks.js` | Hook and golden-hook tests. | Must keep startup bootstrap, create-goal guard, stop continuation, prompt context, steering, scoped roots, and receipt validation fail-closed. |
...
| `src/report.js` | Report and CLI evidence tests. | Must write portable evidence reports with summary counts, timestamps, artifacts, timeline, and next action. |
...
| `src/trace.js` | Loop-gate and CLI evidence tests. | Must show artifact-backed proof, missing proof, suggested paths, ledger timeline, and evidence summary counts. |
...
| `test/doctor.test.js` | `npm test`. | Must cover doctor checks for package, audit, vocabulary, comparison, design decisions, generated install metadata, and reviewability. |
| `test/file-audit.test.js` | `npm test`. | Must prove the file-audit verifier fails stale inventory rows. |
| `test/fleet.test.js` | `npm test`. | Must prove verdict normalization, handoff recording/update, fleet reconciliation, and the parallel-cap warning. |
...
| `test/plugin.test.js` | `npm test`. | Must verify plugin manifest and hook route integrity. |
...
| --- | ---: | --- | --- | --- | --- |
| Turn 0 | 70 | Existing repo had `npm test`, doctor checks, gate notes, and file audit, but source-branded names were part of the contract. | None in this file. | Baseline: strict evidence machinery existed, but naming carried visible lineage. | Remove branded names without dropping gate compatibility. |
...
| Turn 2 | 96 | Targeted gate, docs, audit, and doctor tests. | Runtime modules, docs, tests, skill metadata, and doctor contract. | Renamed public/internal contracts to review gate, matrix gate, design audit, native vocabulary, and generic comparison scan while preserving strict gates. | Reach 100 with fresh marker audit, doctor, and full tests. |
| Turn 3 | 100 | Final `rg` marker audit; final `node src/cli.js doctor --json`; final `npm test`. | Final audit records and inventory rows. | Fresh audit from current files confirms Loopy-native names and preserved gate compatibility. | Maintain the native vocabulary gate as future files change. |
