...
| Skill | 何时使用 | 产出 |
| --- | --- | --- |
| `superloopy-loop` | 使用 `loopy <task>`、`loopy team <task>`、`loopywork`、`$lpy`，或任务必须有证据才能完成时。 | 轻量计划、下一步指引、命令验证证明、质量 gate、最终 evidence report。 |
...
Loop skill 是默认护栏。Research 和 clone 是显式启用的专门模式；它们也必须以 Superloopy evidence 收尾，而不是只给一句完成状态。
