...
test("doctor --json reports Loopy-native packaging, audit, vocabulary, and reviewability checks", () => {
  ...
  assert.equal(parsed.checks.nativeVocabulary.ok, true);
  assert.equal(parsed.checks.nativeVocabulary.policy, "git-visible-files-avoid-source-branded-markers");
  const installReferenceMarker = ["lazy", "codex"].join("");
  assert.deepEqual(parsed.checks.nativeVocabulary.allowedReferences[0], {
    file: "README.md",
    marker: installReferenceMarker,
    pattern: `https:\\/\\/github\\.com\\/code-yeongyu\\/${installReferenceMarker}`
  });
  assert.deepEqual(parsed.checks.nativeVocabulary.findings, []);
  assert.ok(parsed.checks.nativeVocabulary.scanned.includes("src/doctor.js"));
...
test("doctor ignores Codex marketplace install metadata in plugin cache", async () => {
  ...
  assert.equal(result.checks.nativeVocabulary.scanned.includes(".codex-marketplace-install.json"), false);
});
...
test("doctor native vocabulary scan rejects branded source markers", async () => {
  const repo = await tempRepoCopy();
  const goalsPath = join(repo, "src", "goals.js");
  const goals = await readFile(goalsPath, "utf8");
  const marker = ["lazy", "codex"].join("");
  await writeFile(goalsPath, `${goals}\nconst copiedRuntimeMarker = "${marker}";\n`, "utf8");

  const result = await runDoctor(repo);

  assert.equal(result.ok, false);
  assert.equal(result.checks.nativeVocabulary.ok, false);
  assert.equal(result.checks.nativeVocabulary.findings[0].file, "src/goals.js");
  assert.equal(result.checks.nativeVocabulary.findings[0].marker, marker);
});
