...
const GATE_NOTE_SECTIONS = ["Gate Compatibility", "Native Naming", "Golden Scenarios", "Host Contract"];
...
const NATIVE_VOCABULARY_POLICY = "git-visible-files-avoid-source-branded-markers";
const NATIVE_VOCABULARY_MARKERS = [
  ["lazy", "codex"],
  ["ga", "jae"],
  [".o", "mo"],
  [".g", "jc"],
...
const INSTALL_REFERENCE_REPO = ["lazy", "codex"].join("");
const NATIVE_VOCABULARY_ALLOWED_REFERENCES = [
  {
    file: "README.md",
    marker: INSTALL_REFERENCE_REPO,
    pattern: new RegExp(`https:\\/\\/github\\.com\\/code-yeongyu\\/${INSTALL_REFERENCE_REPO}`, "u")
  }
];
const COMPARISON_SOURCES = [{ key: "external", name: "External comparison", audited: null }];
...
async function checkNativeVocabulary(cwd) {
  try {
    const scanned = listGitVisibleFiles(cwd).filter(isNativeVocabularyFile);
    const findings = [];
    for (const file of scanned) {
      const content = await readFile(join(cwd, file), "utf8");
      const lines = content.split("\n");
      lines.forEach((line, index) => {
        const marker = markerInLine(line);
        if (marker !== null && allowedNativeVocabularyReference(file, marker, line)) return;
        if (marker !== null) findings.push({ file, line: index + 1, marker });
      });
    }
    if (findings.length > 0) {
      return {
        ok: false,
        policy: NATIVE_VOCABULARY_POLICY,
        markers: NATIVE_VOCABULARY_MARKERS,
        allowedReferences: formatAllowedNativeVocabularyReferences(),
        scanned,
        findings,
        message: `Git-visible files contain non-native source markers: ${findings.map((item) => `${item.file}:${item.line}:${item.marker}`).join(", ")}.`
      };
    }
    return { ok: true, policy: NATIVE_VOCABULARY_POLICY, markers: NATIVE_VOCABULARY_MARKERS, allowedReferences: formatAllowedNativeVocabularyReferences(), scanned, findings };
  } catch (error) {
    return fail(error instanceof Error ? error.message : String(error));
  }
}
...
function isNativeVocabularyFile(file) {
  return /\.(js|md|json|yaml)$/u.test(file)
    && !file.startsWith(".git/")
...
function markerInLine(line) {
  const lower = line.toLowerCase();
  return NATIVE_VOCABULARY_MARKERS.find((marker) => lower.includes(marker)) ?? null;
}

function allowedNativeVocabularyReference(file, marker, line) {
  return NATIVE_VOCABULARY_ALLOWED_REFERENCES.some((reference) =>
    reference.file === file && reference.marker === marker && reference.pattern.test(line)
  );
}

function formatAllowedNativeVocabularyReferences() {
  return NATIVE_VOCABULARY_ALLOWED_REFERENCES.map((reference) => ({
    file: reference.file,
    marker: reference.marker,
    pattern: reference.pattern.source
  }));
}
